...
class Population:
    def __init__(
        ...
        Args:
          ...
          delta: A float number that defines the minimum distance two genomes require
            to be apart before there sorted into different species. Set to None if
            single species is desired (no speciation).
          genome_seeder: An iterable of genomes that will be used to generate the
            initial population.
          metric: Function that takes two genomes and returns the distance between them.
            The metric is used to partition the genomes into species. If None then no
            speciation will take place.

        Raises:
          ValueError: If delta is not equal to None but metric is.
        """
